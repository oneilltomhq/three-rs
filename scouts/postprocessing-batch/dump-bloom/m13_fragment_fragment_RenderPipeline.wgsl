...
fn reinhardToneMapping ( color : vec3<f32>, exposure : f32 ) -> vec3<f32> {
	...
	return clamp( ( nodeVar0 / ( nodeVar0 + vec3<f32>( 1.0 ) ) ), vec3<f32>( 0.0 ), vec3<f32>( 1.0 ) );
...
fn main( @location( 0 ) nodeVarying0 : vec2<f32> ) -> OutputStruct {
	...
	nodeVar2 = nodeVar1;
	nodeVar2 = nodeVar1;
	nodeVar3 = ( nodeVar0 + nodeVar2 );
	nodeVar4 = fn1( vec4<f32>( nodeVar3.xyz, clamp( nodeVar3.w, 0.0, 1.0 ) ) );
	nodeVar5 = vec4<f32>( reinhardToneMapping( nodeVar4.xyz, render.nodeUniform2 ), nodeVar4.w );
