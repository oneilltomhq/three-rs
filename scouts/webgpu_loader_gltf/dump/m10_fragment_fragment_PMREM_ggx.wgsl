...
fn main( @location( 0 ) nodeVarying4 : vec3<f32> ) -> OutputStruct {
	...
	if ( ( object.nodeUniform0 < 0.001 ) ) {
		...
		nodeVar7.y = ( nodeVar7.y + ( 4.0 * ( exp2( 9.0 ) - nodeVar6 ) ) );
		nodeVar7.x = ( nodeVar7.x * 0.0006510416666666666 );
		nodeVar7.y = ( nodeVar7.y * 0.00048828125 );
		...
		for ( var i : i32 = 0; i < 256; i ++ ) {
			...
			if ( ( nodeVar19 > 0.0 ) ) {
				...
				nodeVar24.y = ( nodeVar24.y + ( 4.0 * ( exp2( 9.0 ) - nodeVar23 ) ) );
				nodeVar24.x = ( nodeVar24.x * 0.0006510416666666666 );
				nodeVar24.y = ( nodeVar24.y * 0.00048828125 );
