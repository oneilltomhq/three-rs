...
fn main( @location( 0 ) normal : vec3<f32>,
	...
	if ( ( render.cameraProjectionMatrix[ 3u ][ 3u ] == 1.0 ) ) {
		...
		nodeVar18 = ( positionLocal * vec3<f32>( ( ( 1.0 / render.cameraProjectionMatrix[ 1u ][ 1u ] ) * 3.0 ) ) );

	} else {

		positionLocal = position;
		nodeVar18 = positionLocal;

	}

	nodeVar19 = ( render.cameraProjectionMatrix * vec4<f32>( ( modelViewMatrix * vec4<f32>( nodeVar18, 0.0 ) ).xyz, 1.0 ) );
	nodeVar20 = vec4<f32>( nodeVar19.x, nodeVar19.y, nodeVar19.w, nodeVar19.w );

	// result

	varyings.builtinClipSpace = nodeVar20;
