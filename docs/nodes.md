...
* **`clearcoatNormalView` assigned before the light loop (§28).** Three
...
## 28. `webgpu_clearcoat` — the direct clearcoat lobe, and the coat's own normal
...
### 28.1 `direct()`'s clearcoat branch
...
### 28.2 `clearcoatNormalView` without a clearcoat normal map is the *geometric* normal
