...
(`src/materials/environment.rs`). The generator's `fromCubemap` path is the
only one ported; `fromScene` and `fromEquirectangular` — and with them the
golden-angle Gaussian blur shader and `BLUR_SAMPLES` — are deferred, because no
example on the ladder reaches them.

The three shaders the rung adds are generated by the node system and match
three's dump line for line, up to the classes §8 already lists (the header
line, the session-global uniform/varying counters, uniform-struct field order,
blank lines after a block): `PMREM_cubemap` against `m01`, `PMREM_ggx` against
`m03`, and the `Background.material` cube-UV read against `m05`. All three are
in `examples/dump_wgsl.rs` as `pmrem_cubemap`, `pmrem_ggx` and
`pmrem_background`.

Three numeric gates sit under the image, in `tests/pmrem.rs`: the GGX roughness
ladder and its `mipInt`s, the atlas rectangles, and `_generateCubeUVSize`. The
oracle is three's own code, driven directly — see the file's header for how.
