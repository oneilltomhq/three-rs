...
silent `PCFSoftShadowMap` are listed in §28.5.
...
## 28. Shadow filters, `filterNode` / `shadowNode`, and VSM (`webgpu_shadowmap_vsm`, `webgpu_shadowmap_pointlight`)
...
### 28.1 What each type changes besides the filter
...
### 28.2 An `RGFormat` texture is a `vec2` node
...
### 28.3 `NodeBuilder.getOutputType()` for an RG target
