...
pub fn rounded_box_geometry_with_groups(
    ...
    geometry.index = None;
    geometry.position = geometry2.position;
    geometry.normal = geometry2.normal;
    geometry.uv = geometry2.uv;
    ...
    let positions = &mut geometry.position.as_mut().unwrap().array;
    ...
    let mut normals = geometry.normal.take().unwrap();
    let mut uvs = geometry.uv.take().unwrap();
    ...
    geometry.normal = Some(normals);
    geometry.uv = Some(uvs);
    (geometry, groups)
}
