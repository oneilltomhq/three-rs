//! The DFG lookup table of `three.js/src/nodes/functions/BSDF/DFGLUT.js`.
//!
//! A 16x16 RG16F `DataTexture`: per texel, the scale and bias of the
//! split-sum specular approximation, 4096 samples each. `PhysicalLightingModel`
//! samples it at `vec2( Roughness, clamp( dot( N, V ) ) )` for both the
//! direct-light multi-scattering compensation and `EnvironmentBRDF`.
//!
//! The literal is three.js' `Uint16Array` verbatim — half-float bit patterns,
//! two per texel, row-major from the top — so the bytes reaching the GPU are
//! the bytes three.js uploads.

use std::cell::RefCell;

use crate::textures::{MinFilter, Texture, TextureFilter};

/// `DATA` — 16 rows of 16 RG texels.
#[rustfmt::skip]
const DATA: [u16; 512] = [
...
thread_local! {
    /// three.js builds the `DataTexture` once, in module scope, and every
    /// physical material shares it — which is why it shows up as one extra
    /// sampler/texture pair in every `MeshStandardMaterial` pipeline.
    static LUT: RefCell<Option<Texture>> = const { RefCell::new(None) };
}

/// `DFGLUT`'s lazily-built `DataTexture`: `RGFormat`, `HalfFloatType`,
/// `LinearFilter` both ways, `ClampToEdgeWrapping`, no mipmaps, and — because
/// it is a `DataTexture` — `flipY = false`.
pub fn dfg_lut() -> Texture {
    LUT.with(|cell| {
        let mut slot = cell.borrow_mut();
        if slot.is_none() {
            let bytes: Vec<u8> = DATA.iter().flat_map(|v| v.to_le_bytes()).collect();
            let texture = Texture::new(16, 16, Some(bytes));
            texture.set_format(wgpu::TextureFormat::Rg16Float);
