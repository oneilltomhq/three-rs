    ...
    fn generate_inner(&mut self, node: &NodeRef) -> String {
        match &*node.0 {
            ...
            Node::Loop {
                ...
                let (start, count, index, update, body) = (
                    start.clone(),
                    count.clone(),
                    index.clone(),
                    update.clone(),
                ...
                // `LoopNode`: `i ++` by default, `i += update` when the loop
                // was given one — `RaymarchingBox`'s float march.
                ...
                self.emit(String::new());
                self.emit(format!(
                    "for ( var {name} : {} = {sstart}; {name} < {scount}; {step} ) {{",
                    wgsl::type_name(ty)
                ));
                self.emit(String::new());
                self.push_scope();
