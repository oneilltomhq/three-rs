...
pub fn check_sample_with_groups(
    ...
    match &sample.position {
        Some(want) => check_attr(expr, "position", geometry.position(), want),
        None => assert!(geometry.position().is_none(), "{expr}: unexpected position"),
    }
    match &sample.normal {
        Some(want) => check_attr(expr, "normal", geometry.normal(), want),
        None => assert!(geometry.normal().is_none(), "{expr}: unexpected normal"),
    }
    match &sample.uv {
        Some(want) => check_attr(expr, "uv", geometry.uv(), want),
...
pub fn bounding_box(geometry: &BufferGeometry) -> ([f64; 3], [f64; 3]) {
    let position = geometry.position().expect("position");
