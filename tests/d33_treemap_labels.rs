...
fn the_layout_matches_the_d33_page() {
    ...
    close("camera.position.x", app.camera.object.position.x, position[0].as_f64().unwrap());
    close("camera.position.y", app.camera.object.position.y, position[1].as_f64().unwrap());
    close("camera.position.z", app.camera.object.position.z, position[2].as_f64().unwrap());

    let quaternion = camera["quaternion"].as_array().unwrap();
    close("camera.quaternion.x", app.camera.object.quaternion.x, quaternion[0].as_f64().unwrap());
    close("camera.quaternion.y", app.camera.object.quaternion.y, quaternion[1].as_f64().unwrap());
    close("camera.quaternion.z", app.camera.object.quaternion.z, quaternion[2].as_f64().unwrap());
    close("camera.quaternion.w", app.camera.object.quaternion.w, quaternion[3].as_f64().unwrap());
